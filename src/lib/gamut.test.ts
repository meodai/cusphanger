...
describe('maxChromaAt', () => {
  it('is 0 at the extremes of lightness', () => {
    expect(maxChromaAt(120, 0, oklchSrgb)).toBe(0);
    expect(maxChromaAt(120, 1, oklchSrgb)).toBe(0);
  });

  it('returns a positive chroma in the mid range', () => {
    expect(maxChromaAt(260, 0.6, oklchSrgb)).toBeGreaterThan(0.05);
  });

  it('keeps the resulting color inside the sRGB gamut', () => {
    const c = maxChromaAt(30, 0.7, oklchSrgb);
  ...
  it('allows more chroma in P3 than in sRGB for a saturated hue', () => {
    const srgb = maxChromaAt(150, 0.7, oklchSrgb);
    const p3 = maxChromaAt(150, 0.7, oklchP3);
...
describe('cusp', () => {
  it('chroma is >= maxChromaAt at any sampled lightness', () => {
    const peak = cusp(260, oklchSrgb);
    for (const l of [0.2, 0.4, 0.6, 0.8]) {
      expect(peak.c + 1e-9).toBeGreaterThanOrEqual(maxChromaAt(260, l, oklchSrgb));
    }
